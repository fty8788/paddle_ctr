...
intent_feature_dims = 3000000
term_feature_dims = 300000
...
id_features = 'query intent q1 q2 q3 q4 q5 q6 q7 q8 q9 q10 i1 i2 i3 i4 i5 i6 i7 i8 i9 i10'.split()
...
def get_all_field_names(mode=0):
    ...
    @return: list of str
    '''
    return categorial_features + id_features + ['click'] \
        if mode == 0 else []
...
for key in id_features:
    ...
    # for normal ID features
        ...
        fields[key] = IDfeatureGenerator(intent_feature_dims)
    else:
        fields[key] = IDfeatureGenerator(term_feature_dims)
...
def detect_dataset(path, topn, id_fea_space=10000):
    ...
    feature_dims['dnn_input'] = np.sum(
        feature_dims[key] for key in categorial_features) + 1
...
class AvazuDataset(object):
    ...
    def _parse(self, path, skip_n_lines=-1, top_n_lines=-1):
        with open(path, 'rb') as csvfile:
            ...
            categorial_dims = [
                feature_dims[key] for key in categorial_features 
            ]
